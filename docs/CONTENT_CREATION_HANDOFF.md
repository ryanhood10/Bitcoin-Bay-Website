...
- **2026-05-06 — Pi-side LANDED (later same day):** `bcbay_research.py:per_platform_topics` schema tightened: `image_subject` MUST be a full searchable noun (no abbreviations, no abstract verbs). Added `image_scene_prompt` + `time_window_hint` fields (optional, backward-compatible) to all per-platform topic levels. Blog/video schemas untouched. The dashboard 🎨 button now has Claude-suggested scene prompts to pre-fill the textarea on humor/observational topics. **The deferred work is now live game-state polling** — see [CONTENT_STRATEGY.md](CONTENT_STRATEGY.md) "Deferred work" for that and other Phase 8 candidates.
- **2026-05-06 — Phase 6 landed (voice + UI + carousel UX):**
  - Pulled live samples from @BitcoinBay_com (last 11 tweets) and @bitcoin_bay (last 15 IG posts). Real voice is meme/reaction-style with heavy emoji, first-name basis with athletes ("OH NO SGA 😳", "Bane doing the most per usual") — completely different from the previous "sharp handicapper" framing the drafter prompts assumed. Replaced single `BB_VOICE` with three platform/variant-specific blocks. Twitter now generates two variants per topic (meme + professional/news) and the operator swaps via ⇄ button.
  - Dropped the 😄 Funny twist button (per operator: "less editing and just have another option" — variant swap is the simpler answer).
  - UI got the visual-density pass: dark Twitter column header / IG gradient header for clear platform separation, image-first IG cards, carousel slide thumbnail strip (each slide visible as a small image instead of text-only tab), hashtag pills with edit-on-click, mobile-friendly stacking.
  - Carousel UX: per-slide drag-to-position headline overlay + color picker (4 BB-palette swatches + native color picker), 🔁 Regen all images, ➕ Add BB CTA slide, 📦 Download all (ZIP). The CTA slide is the ONLY place BB branding (watermark) is acceptable on a regular post.
  - Tests jumped 97 → 113 (4 new endpoints × 4 auth/validation cases each). Heroku release: TBD on the next push.
- **2026-05-08 → 2026-05-13 — Phase 8 + 9.x landed** (multi-day arc; commit ladder `c95623c` → `e5c1f58`):
  - **Phase 8** — "Today's games" panel above the columns (ESPN free API, no key). Operator clicks **📡 Live state** for live score / plays / win-prob, or **✍️ Draft** to seed a one-shot Twitter draft from the live game (~$0.04, operator-triggered).
  - **Phase 9.1** — branded slide layout redesign + mobile-first audit fixes (color-swatch hit areas, hover-none rules, `100dvh` modal heights, safe-area padding, 16px input fonts to suppress iOS zoom).
  - **Phase 9.2** — replace-photo modal + per-source candidate search. PATCH allowlist + `saveDraftImages` `operatorPicked` guard so manually-picked URLs survive subsequent `/regenerate-all-images` cycles instead of being silently overwritten by `findHeroImage`.
  - **Phase 9.3** — inline 🗑 delete-slide corner button (shows when `slides.length > 2`; floor enforced server-side at 2).
  - **Phase 9.4** — focus modal (⛶ corner button → centered ~760px on desktop, full-screen on mobile with `100dvh` + `env(safe-area-inset-bottom)`. Card DOM transplanted in/out so live event handlers stay attached without re-wiring).
  - **Phase 9.5** — branded sticker library. New `brandedOverlays.js` module: 25 transparent-mark SVGs (BB + 14 crypto coins + 10 exchanges) with luminance-aware fill + halo + drop shadow for legibility on any photo. Auto-suggest from `image_subject` keywords with a gold-ringed "Suggested" section in the picker. Server-side bake via `composeOverlayCard({ brandedOverlay })`.
  - **Phase 9.5.1 / 9.5.2** — sticker live-update + transparent style + drag-corner resize handle on both sticker and headline overlays. New `slide.overlay_font_size_pct` field; `composeOverlayCard` bakes the operator-set size into the JPEG so what-you-see is what-publishes.
  - **Phase 9.6** — corner-button tool modals. The 📷 / 🏷️ / 🎨 panels became compact centered modals (mobile: full-screen) instead of inline-stacking under the card. Single shared `.tool-modal-backdrop`, ESC + click-outside dismiss, z-index 10000+ so they sit on top of the focus modal.
  - **Phase 9.7** — card-actions cleanup. Folded the 5-6 button row down to **Save · ⋯ More · Approve**. The "More" popover holds 🆕 New text · ⇄ Switch variant · 🔁 Re-render images · 📦 Download ZIP · ⛔ Skip post. Mobile gets a bottom-sheet with safe-area padding. Sticker keyboard delete: click → highlight → Delete key removes (suppressed when typing in inputs / contenteditable). CTA dedup: "+" tile hides when any `slide_role === 'cta'` exists, preventing operators from layering a second CTA on top of Claude's natural deck-ender.
  - **Phase 9.8 → 9.9** — broad-web image source. Tried Google Custom Search first; spent four cowork-agent investigations confirming the eldrin GCP project (956047495127) has an org-policy block (`commerceorggovernance.marketplaceServices` baseline constraint) that returns 403 `domain="global", reason="forbidden"` regardless of how clean the rest of the config is. Pivoted to **Brave Search Images**: free 2,000 queries/month, single API key (no CSE concept), single header auth (`X-Subscription-Token`). Picker now renders 4 source sections — Wikimedia · Pexels · Unsplash · Brave — with license hints under each label. Google integration code stays env-var-gated/dormant in case the GCP block is ever resolved.
  - **Phase 9.10** — stale-drafts banner. The dashboard previously had no signal that the displayed drafts were week-old (drafter only ran when the operator clicked the button). `GET /post-drafts` now returns `drafts_brief_date`, `latest_brief_date`, and `stale: bool`. Client renders a high-attention orange banner above the columns with a one-click "Generate drafts for [latest brief date]" button.
  - **Phase 9.11** — Pi-cron drafter trigger. Real underlying fix for the stale-drafts root cause: there's no Heroku Scheduler — and we don't need one. The Pi already runs `bcbay_research.py` daily at 06:15. New token-protected `POST /api/cron/run-drafter` endpoint (auth via `BCBAY_CRON_TOKEN` env var, constant-time compare, 503 if env unset, 401 on missing/wrong token, 202 fire-and-forget on success). Pi crontab now chains a curl into this endpoint after writing the brief, so drafts land before the operator wakes up. `runDrafter` also gained a fallback to the latest available brief if today's UTC date isn't in Mongo yet (UTC day-flip race). End-to-end verified: Pi cron → Heroku → 4 fresh drafts in Mongo → dashboard updates.
  - Hashtag-regex hotfix landed mid-arc: `/(#[\w]+)/g` was matching `#39` inside the `&#39;` apostrophe entity and breaking the entity into `&<span>#39</span>;`, which the browser then rendered as the literal character reference. Anchored the regex with `(^|[^&\w])` lookbehind. Mongo data was clean — render path was the bug all along.
  - Tests landed 113 → 146.
