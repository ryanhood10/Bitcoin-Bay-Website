...
| 9.2 ✅ | Replace photo (non-AI) | 2h | `GET /api/admin/dashboard/photo-search?subject=&intent=` returns top 3 candidates per source (Wikimedia / Pexels / Unsplash). Inline panel on the slide image with thumbs + paste-URL. PATCH the slide's `image_url`, trigger /regenerate-all-images. UI: 📷 corner button on every image (carousel slides + Twitter/IG single). **Shipped — 4 new endpoint tests + 4 candidate-search helpers.** |
...
| 9.4 ✅ | Focus modal (X + IG) | 3-4h | Reusable modal component: full-screen on phones (<700px), centered ~760px on desktop. Big preview + grouped edit controls. ESC / backdrop click cancels. Replaces the inline-stacked editor for deep edits; light edits (drag headline, color swatch) stay inline. **Shipped — `⛶` button in every card head, transplant pattern keeps live event handlers, modal-aware loadDrafts skips column repaint.** |
| 9.5 ✅ | Branded overlay library + auto-suggest | 4-5h | 25-mark library (BB + 14 crypto coins + 10 exchanges) defined in `brandedOverlays.js` and rendered as programmatic SVG circles via `/branded-overlays/:key.svg`. Manifest endpoint `/api/admin/dashboard/branded-overlays?subject=` returns the full list + auto-suggested keys based on the subject's keyword match. Sticker picker UI: 6-col (4-col mobile) grid grouped Suggested → BB → Coins → Exchanges. Per-slide HTML overlay rendered draggable; scale slider 8-40%. Save baking via existing `/regenerate-all-images`. `composeOverlayCard` accepts `brandedOverlay: { key, x_pct, y_pct, scale_pct }` and sharp-composites the SVG at saved coords. One mark per slide / per single-image draft. **Shipped — 6 new endpoint tests, 137/137 green.** |
...
- ~~2026-05-06 — **Pi-side TODO** (Phase 1 amendment, blocked on SSH)~~ **LANDED 2026-05-06 (later same day)**: `bcbay_research.py:per_platform_topics` prompt now requires `image_subject` to be a FULL athlete name (no SGA/KAT/CMC) or specific noun, no abstract verb-subjects. New optional fields added to all topic levels (Twitter topics, IG topic, IG carousel slides): `image_scene_prompt` (string OR null) for cases where a real photo of the moment is unlikely (drafter / dashboard 🎨 button uses this for Replicate AI generation), and `time_window_hint` (anytime|pre_tipoff|halftime|post_game) so the dashboard can sort/highlight on game-window timing. Backward-compatible — validator doesn't enforce new fields; downstream falls back to defaults if Claude omits them. Pi script went 1805 → 1813 lines; backup at `~/bcbay/bcbay_research.py.bak-20260506-195557`. Blog and video schemas untouched.
- **2026-05-06 — Phase 8 LANDED: game-state on demand** (re-scoped from auto-polling to operator-pull per operator feedback "rare use, don't make it expensive"). Pi `bcbay_research.py` gains `fetch_todays_games()` — pulls from ESPN free public scoreboard API across NBA/NFL/MLB/NHL/UFC/CFB/MLS, filters to ~10 popular games by big-market-team + national-broadcast scoring, populates `bcb_post_briefs.todays_games`. Heroku gains `GET /api/admin/dashboard/game-state` (ESPN summary proxy with 30s in-memory cache, league_path whitelisted to 7 paths to prevent SSRF) + `POST /api/admin/dashboard/draft-from-game` (FULL role, ~$0.04/call). `contentDrafter.draftFromGameState()` builds a one-shot Twitter draft from the live state with both meme + professional variants. Dashboard gains a "Today's games" panel above the Twitter/IG grid — horizontal strip of game cards with **📡 Live state** (expand inline with score + last plays + win prob) and **✍️ Draft** (~$0.04, seeds Claude with the game state). Cost: $0 if operator doesn't click anything. 8 new endpoint tests; total now 121.
- **2026-05-06 — Phase 6 landed**: voice refresh from live account analysis + variant swap + UI density/separation + carousel UX (drag editor, color picker, regen-all-images, BB CTA slide append, ZIP download).
  - Phase 6.1 (voice): replaced single `BB_VOICE` with three platform/variant-specific blocks (`BB_VOICE_TWITTER_MEME`, `BB_VOICE_TWITTER_PROFESSIONAL`, `BB_VOICE_IG`) anchored on real samples pulled from @BitcoinBay_com (last 11 tweets) and @bitcoin_bay (last 15 IG posts). Twitter voice is now meme/reaction-style by default (3-12 words, emoji OK, first-name basis); professional/news is the swap-to alternative. IG caption target dropped 600-1100 → 50-250 chars; hashtags dropped 10-15 → 5-7. Carousel deck caption dropped 600-1200 → 100-300 chars; per-slide body_text ≤30 → ≤20 words.
  - Phase 6.2 (variants): `runDrafter` now makes 2 parallel Claude calls per Twitter topic (meme + professional), stores both as `variants[]` with `active_variant_index`. New `POST /swap-variant` endpoint flips the active variant with no Claude call (mirrors top-level fields). UI gains a `MEME` / `NEWS` chip + ⇄ swap button. The 😄 Funny twist button is dropped (operator chose simpler over more options).
  - Phase 6.3 (UI): clearer visual separation — Twitter column gets a dark X-style sticky header, Instagram column gets the IG brand gradient. Card density tightened (smaller padding, smaller image previews, hashtag pills with edit-on-click, advanced expander hides image-URL override). IG cards now image-first (image at top of body before caption). Carousels render a horizontal thumbnail strip with actual slide images instead of text-only tabs. Posted/skipped cards collapse to a one-line preview. Mobile breakpoint tightened (cards stack at <800px, image previews shrink).
  - Phase 6.4 (carousel UX): per-slide drag-to-position headline overlay (mouse + touch), 4 BB-palette color swatches + native `<input type="color">` for the headline color. New endpoints: `POST /regenerate-all-images` (fire-and-forget; re-runs `imageRenderer.saveDraftImages` so drag/color edits land in the rendered composite), `POST /add-cta-slide` (appends a BB-branded CTA slide via `composeBrandedCard`; the only place watermarks are appropriate). `composeOverlayCard` accepts new optional `overlayX`, `overlayY`, `overlayColor` params (% coords + hex color); uses a stroke-shadow for readability when custom-positioned (no gradient). Per-slide download via browser-native `<a download>`.
  - Phase 6.5 (downloads): `GET /post-drafts/:id/zip` streams a ZIP of all carousel slide JPEGs via `archiver`. Notes the Heroku ephemeral-FS caveat (404 if dyno restarted since last render).
  - Tests: +16 new endpoint tests (4 new endpoints × 4 cases each). Total: 97 → 113.
