...
5. **Content drafter** — `/admin/dashboard/content` (full-role only) + `/api/admin/dashboard/post-drafts/*`. Daily X + IG post drafts generated by [contentDrafter.js](contentDrafter.js) from the Pi's morning research brief. Pi cron at 06:15 chains a curl into [POST /api/cron/run-drafter](adminDashboard.js) (token-protected) so drafts land before the operator wakes up — no Heroku Scheduler addon required. Operator reviews each card via the three-button row (💾 Save · ⋯ More · ✓ Approve), opens any card in the **focus modal (⛶)** for full-screen deep editing, and uses corner-button **tool modals** on the image: 📷 replace photo (4 sources — Wikimedia · Pexels · Unsplash · Brave Search), 🏷️ branded sticker (25 crypto/exchange logos with auto-suggest from subject keywords), 🎨 AI scene (Replicate InstantID, ~$0.05/call, operator-triggered, audit-logged), 🗑 delete-slide (carousel only, floor at 2 slides). Headline overlays and stickers are draggable (move) + resizable (gold corner handle). Selected sticker can be removed with the Delete key. A **stale-drafts banner** appears above the columns when the latest brief is newer than the latest draft (one-click re-trigger). Voice is anchored on the live @BitcoinBay_com / @bitcoin_bay accounts (meme-leaning, hype-fan, emoji-friendly). NO watermarks on regular posts; the BB CTA slide is the only branded slot. **Phase 8 — "Today's games" panel** above the columns shows the day's most popular live games (ESPN free API, no key, populated nightly by Pi). Operator clicks **📡 Live state** for live score + plays + win-prob, or **✍️ Draft** to generate a one-shot Twitter draft seeded from the live game state (~$0.04, operator-triggered only). See **[docs/CONTENT_CREATION_HANDOFF.md](docs/CONTENT_CREATION_HANDOFF.md)** + **[docs/CONTENT_CREATION_PLAN.md](docs/CONTENT_CREATION_PLAN.md)** + **[docs/CONTENT_STRATEGY.md](docs/CONTENT_STRATEGY.md)**.
...
adminDashboard.js    ← Express router for /admin/dashboard + /api/admin/dashboard/*.
                       ...
                       photo-search, branded-overlays, swap-variant, add-cta-
                       slide, delete-slide, regenerate-all-images, ZIP, plus
                       the Pi-cron POST /api/cron/run-drafter token endpoint).
contentDrafter.js    ← Daily X + IG post drafter. Reads bcb_post_briefs,
                       writes bcb_post_drafts. Three Claude prompts:
                       twitter, instagram_single, instagram_carousel. Falls
                       back to the latest available brief if today's isn't
                       in Mongo yet (UTC day-flip race). CLI:
                       `node contentDrafter.js --date YYYY-MM-DD [--dry-run]`.
imageRenderer.js     ← Real-photo cascade (Wikimedia → Unsplash → Pexels →
                       Brave) + BB-branded sharp composites + Replicate
                       InstantID AI scene generation. Operator-picked
                       photos (image_source ∈ {wikimedia,pexels,unsplash,
                       brave,google,manual}) survive the regen-images
                       cycle. Helpers: inferIntent, isBBSubject,
                       inferBrandedKind, pexelsOffTopic, plus per-source
                       `searchXxxCandidates` for the replace-photo modal.
brandedOverlays.js   ← Phase 9.5 sticker library. 25 crypto/exchange logos
                       (BB + 14 coins + 10 exchanges) rendered as
                       transparent SVG marks with halo + drop-shadow for
                       photo legibility. Auto-suggest based on subject
                       keywords. Served at /branded-overlays/:key.svg.
...
views/content-drafts.html ← Full SPA for the content-drafter review/approve
                       surface. Three-button card actions (Save · ⋯ More
                       · Approve), focus modal (⛶), corner-button tool
                       modals (📷 photo · 🏷️ sticker · 🎨 AI scene · 🗑
                       delete-slide), in-canvas drag-and-resize for both
                       headline overlay and stickers, click+Delete to
                       remove selected sticker, stale-drafts banner with
                       one-click re-trigger. Full-role admins only.
...
scripts/clean-draft-entities.js ← One-shot Mongo scrub of literal HTML
                       entities in existing post drafts (multi-pass decode).
scripts/diag-mongo-state.js ← Read-only diagnostic of bcb_post_briefs +
                       bcb_post_drafts state (date distribution, latest
                       per collection). Used to spot stale-drafter bugs.
scripts/test-image-search.js ← Local smoke test for the Brave + Google
                       image search APIs. Reads BRAVE_API_KEY / GOOGLE_*
                       from env, prints actual results or full error.
tests/*.test.js      ← node:test suite — auth module, dashboard API, IG OAuth,
                       bonus calculator, admin content endpoints, image
                       renderer helpers, game-state proxy, branded-overlay
                       picker, Pi-cron drafter trigger, Brave guard rails.
                       Run: npm test (146 tests).
