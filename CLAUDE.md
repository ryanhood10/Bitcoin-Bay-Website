...
tests/*.test.js      ← node:test suite — auth module, dashboard API, IG OAuth,
                       ...
                       renderer helpers. Run: npm test (113 tests).
...
| `bcb_post_drafts`    | Per-platform post drafts (twitter / instagram_single / instagram_carousel). Lifecycle: draft → approved → posted/skipped. Read + edited from `/admin/dashboard/content`. | auto |
