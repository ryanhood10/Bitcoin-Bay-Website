...
5. **Content drafter** — `/admin/dashboard/content` (full-role only) + `/api/admin/dashboard/post-drafts/*`. Daily X + IG post drafts generated by [contentDrafter.js](contentDrafter.js) from the Pi's morning research brief. Operator reviews each card, edits text, regenerates with optional humor pass, and approves to publish. Hero images come from a real-photo cascade (Wikimedia → Unsplash → Pexels) with BB-branded `sharp` composites for promo posts and an opt-in **🎨 Generate scene** button that calls Replicate InstantID for AI scenes with the actual athlete's face (operator-triggered, ~$0.05/call, audit-logged). NO watermarks on regular posts. See **[docs/CONTENT_CREATION_HANDOFF.md](docs/CONTENT_CREATION_HANDOFF.md)** + **[docs/CONTENT_CREATION_PLAN.md](docs/CONTENT_CREATION_PLAN.md)**.
...
tests/*.test.js      ← node:test suite — auth module, dashboard API, IG OAuth,
                       ...
                       renderer helpers. Run: npm test (97 tests).
